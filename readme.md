...
- 无人检测：定时检测服务器在线人数，同一服务器连续 Y 次无人在线时通报并提醒（默认关闭，仅发送纯文本提醒，不再 @ 任何人）
- 自动开服：定时检测服务器状态，自动启动已关闭的服务器；连续 3 次开服后仍处于关闭状态时自动执行强制重启（stop → kill → start）（默认关闭）
...
| 字段 | 说明 | 默认值 |
| --- | --- | --- |
| `emptyDetectEnabled` | 是否开启无人检测 | `false` |
| `emptyDetectIntervalMinutes` | 检测间隔 X（分钟），每隔 X 分钟对所有服务器检测一次在线人数（关闭的服务器不参与统计） | `60` |
| `emptyDetectThreshold` | 连续无人在线次数阈值 Y，同一服务器连续 Y 次检测均为 0 人在线时通报并提醒，随后计数清零，等待下一计数循环 | `48` |
| `emptyDetectNotifyChannels` | 通报发送的群号 / 频道 ID 列表 | - |

### 自动开服

| 字段 | 说明 | 默认值 |
| --- | --- | --- |
| `autoStartEnabled` | 是否开启自动开服 | `false` |
| `autoStartIntervalMinutes` | 检测间隔 X（分钟），每隔 X 分钟对所有服务器检测一次，检测到关闭的服务器（配置了 `minekuaiInstanceId`）自动发送一次开服指令 | `10` |

自动开服说明：同一服务器连续 3 次检测均处于关闭状态（即已执行 3 次开服指令后依旧未启动成功）时，自动执行一次强制重启指令（stop → kill → start），随后重新开始计数；服务器恢复正常在线后计数自动清零。
